...
export type PublicVariant = Pick<
  ...
  "id" | "size" | "color" | "stock_qty" | "is_active"
>;
...
export async function listPublicProducts(
  ...
  let query = supabase
    .from("products")
    .select(
      "id, tenant_id, category_id, title, description, price, old_price, images, is_featured, sort_order, product_variants(id, size, color, stock_qty, is_active)",
    )
    .eq("tenant_id", tenantId)
    .eq("is_active", true)
    .order("sort_order", { ascending: true })
